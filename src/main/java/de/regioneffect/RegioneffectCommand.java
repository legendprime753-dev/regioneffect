...
public final class RegioneffectCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage("§eVerwendung: /regioneffect <create|effect|remove|reload>");
            return true;
        }
    ...
    private boolean handleCreate(CommandSender sender, String[] args) {
        ...
        if (!REGION_NAME_PATTERN.matcher(regionName).matches()) {
            sender.sendMessage("§cRegionsnamen dürfen nur Buchstaben, Zahlen, '-' und '_' enthalten.");
            return true;
        }
        ...
        boolean created = plugin.getRegionManager().createRegion(regionName, pos1, pos2).isPresent();
        if (!created) {
            sender.sendMessage("§cEine Region mit diesem Namen existiert bereits.");
            return true;
        }

        sender.sendMessage("§aRegion '§f" + regionName + "§a' wurde gespeichert.");
    ...
    private boolean handleEffect(CommandSender sender, String[] args) {
        ...
        sender.sendMessage(
                "§aEffekt für Region '§f" + args[1] + "§a' gesetzt: §f"
                        + PotionEffectResolver.displayName(type)
                        + " §8[" + PotionEffectResolver.toStorageKey(type) + "] §7(Stärke " + amplifier + ")"
        );
        plugin.getEffectService().refreshAll(plugin.getServer().getOnlinePlayers());
        return true;
    }
    ...
    private boolean handleRemove(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage("§eVerwendung: /region remove <Name>");
            return true;
        }
        ...
        sender.sendMessage("§aRegion '§f" + args[1] + "§a' wurde entfernt.");
        plugin.getEffectService().refreshAll(plugin.getServer().getOnlinePlayers());
        return true;
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            return filter(List.of("create", "effect", "remove", "reload"), args[0]);
        }
        if (args.length == 2 && (args[0].equalsIgnoreCase("effect") || args[0].equalsIgnoreCase("remove"))) {
